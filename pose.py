...
def filter_keypoints(keypoints_tensor):
    ...
    mask = (conf >= keypoint_conf_threshold) & (
        (keypoints_tensor[:, 0] != 0) | (keypoints_tensor[:, 1] != 0))
    # 应用掩码过滤关键点，保留符合条件的点
    ...
    if result.boxes:
        for box, keypoints in zip(result.boxes, result.keypoints.data):
            ...
            for i, (x, y, kp_conf) in enumerate(filtered_keypoints):
                print(
                    f"  Keypoint {i} ({COCO_KEYPOINTS[i] if i < len(COCO_KEYPOINTS) else 'unknown'}): ({x}, {y}), Confidence: {kp_conf}")
...
def advise_pose(target_mod, reference_mod, target_img_with_box, target_width, target_height, reference_width, reference_height):
    ...
    reference_keypoints = [
        normalize_coordinates(x, y, reference_width,
                              reference_height) + (conf,)
    ...
    for i, (target_x, target_y, target_conf) in enumerate(target_keypoints):
        if i >= len(reference_keypoints):
            print(
                f"Skipping Keypoint {i} due to reference keypoints shortage.")
            continue
        ...
        print(
            f"Keypoint {i} ({COCO_KEYPOINTS[i] if i < len(COCO_KEYPOINTS) else 'unknown'}): Move x: {dx}, Move y: {dy}")

        # 将建议坐标反归一化以绘制箭头
        target_x_denorm, target_y_denorm = denormalize_coordinates(
            target_x, target_y, target_width, target_height)
        reference_x_denorm, reference_y_denorm = denormalize_coordinates(
            reference_x, reference_y, target_width, target_height)
...
if __name__ == "__main__":
    ...
    for target_obj in target_mod:
        ...
        if reference_mod:
            ...
            print(
                f"Image with pose advice saved at {output_image_with_pose_path}")
        else:
